...
test("applies migrations in filename order without reordering the input", async () => {
	...
	expect(
		await sql.unsafe<Array<{ id: number }>>("select * from entries"),
	).toEqual([{ id: 1 }]);
...
test("initializes the application schema and skips applied migrations", async () => {
	const migrations = [
		{
			...
			sql: await Bun.file(
				new URL("./migrations/0001_call_log.sql", import.meta.url),
			).text(),
		},
	];

	await runMigrations(sql, migrations);
	await sql.unsafe("insert into call_log (created_at) values ($1)", [
	...
	expect(
		await sql.unsafe<Array<{ name: string; applied_at: string }>>(
			"select * from __migrations",
		),
	).toEqual(applied);
	expect(
		await sql.unsafe<Array<{ id: number; created_at: string }>>(
...
test("rolls back a failed migration and permits retrying it", async () => {
	...
	expect(
		await sql.unsafe<Array<{ name: string }>>("select name from __migrations"),
	).toEqual([]);
	expect(
